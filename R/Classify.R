...
classify <- function(FPKM,thresh=c(0.25,0.5,0.5,0.75)){
    ...
    #Anything with prob below threshold should be 'Unclassified'
    #Maj = apply(RFprob[,1:3],1,max)
    #threshclass = data.frame(class = factor(RFpred,levels=c(levels(RFpred),'Unclassified')),row.names= colnames(FPKM))
    #for(i in 1:length(Maj))
    #{
    #    if(Maj[i]> thresh){
    #        threshclass[i,1] = RFpred[i]
    #    }
    #    else
    #    {
    #        threshclass[i,1] = "Unclassified"
    #    }
    #}
